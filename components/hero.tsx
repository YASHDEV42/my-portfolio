...
export function Hero() {
  ...
  const textY = useTransform(scrollYProgress, [0, 1], reduce ? [0, 0] : [0, 120])
  ...
  return (
    <section
      ...
      className="relative flex min-h-[100svh] flex-col justify-center overflow-hidden px-5 pb-16 pt-28 md:px-8"
      style={{
        background:
    ...
    >
      ...
      <motion.div style={{ y: textY, opacity: fade }} className="relative mx-auto w-full max-w-6xl">
        <motion.div
        ...
        >
          ...
          <span className="label text-muted-foreground">{site.location}</span>
        </motion.div>

        <h1 className="font-serif text-[clamp(2.75rem,9vw,7.5rem)] font-medium leading-[0.92] tracking-[-0.02em] text-balance">
          <RevealLines lines={hero.headline} accentIndex={1} />
        </h1>
        ...
        >
          {hero.stats.map((s) => (
            <div key={s.label}>
              <dt className="font-serif text-2xl font-semibold md:text-3xl">{s.value}</dt>
